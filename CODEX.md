...
- 后台 2.0 A7-3 已上线：`/admin/customers` 已接入 `AdminSectionShell`，升级为顶部导航 + 左侧客户与线索功能树 + 右侧工作区；左侧包含客户概览、新线索、全部线索、跟进中、已报价、已成交、已关闭、待处理和规划中项。`operator` 不显示会员管理、后台账号、站点设置、Legacy / 维护入口；本轮未改 `/admin/content`、`/admin/site`、`/admin/status`、旧 `/admin/leads`、API、数据库或 `/global`，后续按 A7-4 扩展 `/admin/status`。
