...
## B70 Homepage hero fold alignment (2026-06-03)

- Code commit: `ed3a1cb fix(home): push credentials below hero fold`
- Full SHA: `ed3a1cb337b1aec5e9e67e52cbe82d87820b7383`
- Vercel deployment: `dpl_GKoPpn6Bdv8zT1VJFUzX66MPVF1d`
- Deployment URL: `https://vessel303-pomf9mxhl-vessel303.vercel.app`
- Status: `READY`
- Scope: B70 was a homepage display-template fix after 09 comparison found that `en.303vessel.cn` keeps the first viewport focused on the full-bleed hero and carousel/contact controls, while vessel303 still let the credentials section intrude too strongly into the first screen. No homepage copy, image URL, CTA label, contact value, product fact, backend content, module order, or `/global` behavior was hardcoded in the frontend.
- Frontend display change: `HomePageContent` now lets the desktop hero run to roughly the viewport fold and removes the negative overlap from the credentials bar, so the backend-published credentials content starts below the first-screen hero instead of visually competing with it.
- Backend-control boundary: all visible homepage text, images, CTA labels, links, proof values, credentials values and product-card content still come from published Home page modules and site configuration. The frontend only controls hero minimum height, credentials-bar placement, background continuity and responsive fold rhythm.
- Validation: `git diff --check`, targeted eslint for `src/components/pages/HomePageContent.tsx`, `npx.cmd tsc --noEmit`, `npm.cmd run audit:public-content`, `npm.cmd run audit:published-content`, `npm.cmd run audit:production-links`, and `npx.cmd next build --webpack` passed. The published-content and production-link audits required approved network reruns after sandbox HTTP 0 failures. Build logs only showed existing PostgreSQL SSL warnings, sandbox database `EACCES` fallback logs, and the known `/global` edge-runtime warning.
- Online checks: Vercel reached `READY`; `https://www.vessel303.com/?lang=en&b70=ed3a1cb` returned 200, `/global?b70=ed3a1cb` returned 200, and unauthenticated `/admin` redirected to `/admin/login` and returned 200. Online desktop Browser screenshot at 1920 x 889 confirmed no horizontal overflow, hero height 918 px, credentials top at 918 px, and the first credentials section below the viewport fold.
- Local visual limitation: local dev rendering did not show published Home modules because sandbox DB access returned PostgreSQL `EACCES`; a sandbox-outside dev-server rerun was rejected because it could execute schema-creation queries against the shared database. Browser also remained read-only for production DOM style injection.
- Mobile limitation: the current Browser path has no viewport/resize capability, so B70 did not produce a real mobile screenshot. Mobile behavior remains code-reviewed and covered by build/type/audit checks, but should be visually rechecked by 09 / 07 when a viewport-capable browser path is available.
- `/global` boundary: no `/global`, MapLibre, MapTiler or `/api/map` diff. Global Contact / Products old 303 exception remains unchanged.
- Next step: 09 should recompare the live homepage against `en.303vessel.cn`, with emphasis on the remaining first-screen gap: the hero proof strip is still visible inside the hero while the reference first screen stays cleaner. Display-template gaps go to 01; backend content/material gaps go to 02 / 03; mobile/image performance regressions go to 07.

## B71 Homepage hero proof rail alignment (2026-06-03)

- Code commit: `cae7bc5 fix(home): lighten hero proof rail`
- Full SHA: `cae7bc533955b5d2362fb25bd3d78f64f4a65512`
- Vercel deployment: `dpl_7NDMHN4BXGoShJY4YRnFjh4sifVp`
- Deployment URL: `https://vessel303-oa1z84ca9-vessel303.vercel.app`
- Status: `READY`
- Scope: B71 was a homepage display-template fix after 09 comparison found that the B70 first-screen fold was close, but the hero proof strip still read too much like a card row while `en.303vessel.cn` keeps the first viewport cleaner around the hero image, title, carousel preview and contact controls. No homepage copy, image URL, CTA label, contact value, product fact, backend content, module order, or `/global` behavior was hardcoded in the frontend.
- Frontend display change: `HomePageContent` now renders the hero proof items as a lighter flex rail instead of a grid of cards, lowers the visible proof value/label scale, removes the visible proof body from the hero rail, and keeps each proof body's `content_*` field as an `sr-only` backend-editing anchor so the visual editor can still locate the field without restoring a heavy first-screen information panel.
- Backend-control boundary: all visible homepage text, images, CTA labels, links, proof values, proof labels, credentials values and product-card content still come from published Home page modules and site configuration. The frontend only controls proof rail density, typography scale, wrapping, visibility and edit-field anchoring.
- 02 backend check: Home still reads published `home` page modules through `listPublishedPageModules('home')`; `home:hero` owns hero slides, CTAs and proof items; `home:credentials` owns the credentials bar. The existing 300 backend tabs were identified read-only; no click, input, draft, publish or backend mutation was performed.
- Validation: `git diff --check`, targeted eslint for `src/components/pages/HomePageContent.tsx`, `npx.cmd tsc --noEmit`, `npm.cmd run audit:public-content`, `npm.cmd run audit:published-content`, `npm.cmd run audit:production-links`, and `npx.cmd next build --webpack` passed. The published-content and production-link audits required approved network runs. The local build exited 0; logs only showed existing PostgreSQL SSL warnings, sandbox database `EACCES` fallback logs, and the known `/global` edge-runtime warning.
- Online checks: Vercel reached `READY`; `https://www.vessel303.com/?lang=en&b71=cae7bc5` returned 200, `/global?b71=cae7bc5` returned 200, and unauthenticated `/admin?b71=cae7bc5` redirected to `/admin/login` and returned 200.
- 09 visual recheck: online Browser screenshot at 1920 x 889 confirmed no horizontal overflow, hero height 918 px, credentials top at 918 px, and the first sections remain hero, credentials, product systems. The proof rail now measures about 688 x 45 px instead of the previous large proof card strip, with visible proof values/labels still backend-driven.
...
## B73 Homepage credentials rhythm compaction (2026-06-03)

- Code commit: `993c421 fix(home): compact credentials rhythm`
- Full SHA: `993c42171b79009e32c3f9e0729e5e9aa4138cbb`
- Vercel deployment: `dpl_A1yaS4jfmJ6aNdLdEHAEeymbtvw2`
- Deployment URL: `https://vessel303-6cxcdml0i-vessel303.vercel.app`
- Status: `READY`
- Scope: B73 was a homepage display-template fix after 09 comparison found that `en.303vessel.cn` moves from the hero into large product visuals, while vessel303 still had a light standalone credentials card that visually interrupted the hero-to-product rhythm. No homepage copy, image URL, CTA label, contact value, product fact, backend content, module order, or `/global` behavior was hardcoded in the frontend.
- Frontend display change: `HomePageContent` now renders `home:credentials` as a compact dark proof band instead of a light floating statistics card. The band keeps the backend-published values and labels, reduces the vertical break between hero and product systems, and uses a 2-column mobile / 4-column desktop grid.
- Backend-control boundary: all visible homepage headlines, subtitles, CTAs, slide images, proof values, proof labels, credentials values, contact entries and product-card content still come from published Home page modules and site configuration. The frontend only controls credentials-band background, spacing, border rhythm, type scale, and responsive column layout.
- 02 backend check: Home still reads published `home` page modules through `listPublishedPageModules('home')`; `home:hero` owns hero slides, CTAs and proof items; `home:credentials` owns the credentials values. Existing 300 backend tabs were treated read-only; no click, input, draft, publish or backend mutation was performed.
- Validation: `git diff --check`, targeted eslint for `src/components/pages/HomePageContent.tsx`, `npx.cmd tsc --noEmit`, `npm.cmd run audit:public-content`, `npm.cmd run audit:published-content`, `npm.cmd run audit:production-links`, and `npx.cmd next build --webpack` passed. Published-content and production-link audits needed approved network runs. Build logs only showed existing PostgreSQL SSL warnings, sandbox database `EACCES` fallback logs, and the known `/global` edge-runtime warning.
- Online checks: Vercel reached `READY`; `https://www.vessel303.com/?lang=en&b73=993c421` returned 200, `/global?b73=993c421` returned 200, and unauthenticated `/admin?b73=993c421` redirected to `/admin/login`.
- 09 visual recheck: online Browser at 1920 x 945 confirmed no horizontal overflow, the credentials band is now dark (`rgb(20, 16, 14)`), about 106 px tall, and the next visible module is `home:large-product-cards`. The hero, credentials and product systems remain backend-module driven.
- Local visual limitation: local dev rendering could not load published Home modules because sandbox DB access returned PostgreSQL `EACCES`; a sandbox-outside dev-server rerun was intentionally not used because it could execute schema or analytics writes against the shared database.
