...
- 08 可视化页面编辑器：`/admin/pages/visual`、页面模块可视化预览、受控字段编辑、模块高亮、点击定位、草稿预览 / 发布上线、快照恢复、模块内 item 管理和后续运营使用规范；不负责普通后台 A/B 包、产品 / 项目 CMS、`/global` 地图、会员支付。
...
- `/admin/pages/visual`：08 C2 已上线，主流程为“保存草稿 -> 草稿预览 -> 发布上线”；普通访客只读取线上版本，后台登录用户可预览草稿；发布草稿才写入线上 `page_modules`，保存草稿不影响前台；快照恢复现在恢复到草稿，不直接影响前台；媒体引用统计和删除保护已纳入 `page_module_drafts` 草稿引用和 `page_module_snapshots` 历史快照引用；原 `/admin/pages` 备用表单仍直接保存线上版本；当前仍不支持整页模块级新增 / 删除 / 排序、自由样式 / 自由布局。
...
- 08 可视化页面编辑器：C2 草稿 / 发布分离已上线，当前主流程是“保存草稿 -> 草稿预览 -> 发布上线”；图片真实上传和无引用媒体删除成功路径仍需单独授权测试；当前仍不支持整页模块级新增 / 删除 / 排序、自由样式 / 自由布局；下一步先补 08 运营使用规范，不继续扩大编辑能力；`/global` edge runtime warning 仍归 04 地图专项。
...
- 08 可视化页面编辑器：C2 草稿 / 发布分离已上线；下一步先补 08 运营使用规范，再评估页面级版本管理、差异对比、图片上传端到端、无引用媒体删除成功路径测试，以及整页模块级新增 / 删除 / 排序。
