...
产品中心 / 产品管理：先做 1 个标准产品详情页模型，不急着恢复或导入全部 39 SKU。后台 B4-0 至 B4-6 已参照 300.cn 产品管理心智完成安全核心能力：产品分类管理、列表分类筛选、低风险批量转分类、产品回收站恢复为草稿、单篇产品 SEO 字段和前台产品详情 metadata 读取。字段和详情页结构确认后，再批量导入后台 CMS。缺失字段不显示，不用 TBD 硬填。
...
- 02 后台运营 / 产品：B4-0 至 B4-6 产品管理 2.0 核心运营能力已完成，已接入产品分类、列表分类筛选、低风险批量转分类、回收站恢复为草稿和 SEO 字段治理第一阶段。下一步建议先做 B4-7 产品生产全链路真实回归，05 以运营人员角色创建一个 Wynne 明确授权的测试产品，验证草稿、分类、SEO、发布、前台详情、取消发布、软删除、回收站恢复和最终清理；权限矩阵仍放到基本建站标准完成后再做。
