...
- 08 可视化页面编辑器：`/admin/pages/visual`、页面模块可视化预览、模块高亮、点击定位、后续受控编辑方案；不负责普通后台 A/B 包、产品 / 项目 CMS、`/global` 地图、会员支付。
...
- `/admin/pages/visual`：08 C1-1 只读可视化预览已上线，支持 Home / About、同源 iframe 真实预览、模块列表、模块高亮、点击定位、跳转 `/admin/pages?module=...`；当前不支持编辑、保存、上传、新增、删除、排序或内联编辑。
...
- 08 可视化页面编辑器：C1-1 只读可视化预览已上线；C1-2 受控文字编辑尚未开放，图片编辑、模块新增 / 删除 / 排序、草稿 / 发布、版本回滚均属后续阶段；`/global` edge runtime warning 仍归 04 地图专项。
...
- 08 可视化页面编辑器：C1-1 只读预览已上线；后续先评估受控文字编辑，不开放自由编辑、保存、上传或模块结构改动。
