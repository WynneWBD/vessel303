...
产品中心 / 产品管理：先做 1 个标准产品详情页模型，不急着恢复或导入全部 39 SKU。后台 B4-0 至 B4-8 已参照 300.cn 产品管理心智完成安全核心能力：产品分类管理、列表分类筛选、低风险批量转分类、产品回收站恢复为草稿、新版产品列表移入回收站、单篇产品 SEO 字段和前台产品详情 metadata 读取；B4-7 已完成生产环境真实全链路回归。字段和详情页结构确认后，再批量导入后台 CMS。缺失字段不显示，不用 TBD 硬填。
...
- 02 后台运营 / 产品：B4-0 至 B4-8 产品管理 2.0 核心运营能力已完成，已接入产品分类、列表分类筛选、低风险批量转分类、回收站恢复为草稿、新版列表移入回收站入口、SEO 字段治理第一阶段和生产全链路真实回归。权限矩阵仍放到基本建站标准完成后再做；下一步可在 300 对照后继续拆产品属性模板、标签 / 标记、品牌、筛选或橱窗管理，仍不混入价格、会员价、代理价、订单、支付和 `/global` 底层。
