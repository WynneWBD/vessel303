...
- 02 后台运营 / 产品：B4-0 至 B4-16 产品管理 2.0 核心运营能力与生产真实运营回归已完成；B7 已完成产品中心前后台 300 对齐深改，`/products` 已升级为 300 式目录页，`/products/[slug]` 通用详情模板已支持图库、价格展示、商务条款、分类、关键词、相关产品和询盘入口，后台产品表单 / 列表 / 经营中心已支持价格展示文本、商务条款、关键词、相关产品和缺项提示。价格仅展示；权限矩阵仍放到基本建站标准完成后再做；不混入会员价、代理价、订单、支付和 `/global` 底层。
...
- 后台运营体验 A / B 包：A1-A6、B1-B7、新 B6 运营数据中心和 B6-7 访问分析准备页已上线；媒体真实上传 / 删除端到端测试仍需单独授权。
