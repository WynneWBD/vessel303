...
- `/admin/products`：产品 CMS 已接入产品列表和通用详情页，支持新建、编辑、发布/下架、删除、筛选、图片选择、前台预览、详情介绍、详情图库和规格参数；固定精细详情页如 `e7`、`v9-gen6` 仍保留原页面。
...
- Step 8：产品 CMS 基础发布链路，含公开详情 slug 校验、后台预览、通用详情介绍、图库和规格参数。
...
- 产品 CMS 下一阶段：增加高级详情模块、图库选择器、规格模板和产品复制能力，减少对固定精细页的依赖。
