...
- 后台 2.0 阶段 A 已开始落地：`/admin` 已脱离旧 `AdminShell` 并重建为运营控制台，`/admin/legacy` 是 admin-only 旧后台维护入口；旧 `/admin/pages`、`/admin/users`、`/admin/settings` 保持 admin-only，`operator` 过渡期仍可访问 `/admin/products`、`/admin/projects`、`/admin/news`、`/admin/leads`、`/admin/media`，旧内容路由暂不迁移。A1.2 / A1.2.1 已让 `/admin` 更接近 300.cn 操作心智，并完成运营首页去重与版式精简；A3 已上线 `/admin/site` 网站管理二级入口页，`/admin` 的网站管理入口已指向 `/admin/site`，`/admin/pages/visual` 是编辑网站主入口，`/admin/media` 是图片管理入口，`/global` 只是查看入口；`operator` 可进入 `/admin/site`，但不显示维护中心、表单模式、设置、账号和 Legacy，`admin` 可见维护中心和 admin-only 维护入口；A3 不改数据库、API、旧后台业务页或 `/global` 地图底层；后台 2.0 继续遵守“复用旧数据、不复用旧交互”，支付、订单、会员价、代理价、国家版本暂不做；`/global` warning 仍归 04。
