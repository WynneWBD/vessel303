...
- 08 可视化页面编辑器：`/admin/pages/visual`、页面模块可视化预览、受控字段编辑、模块高亮、点击定位、草稿预览 / 发布上线、发布前检查、差异摘要、快照恢复、模块内 item 管理和后续运营使用规范；不负责普通后台 A/B 包、产品 / 项目 CMS、`/global` 地图、会员支付。
...
- `/admin/pages/visual`：08 C3 已上线，在 C2 草稿 / 发布分离基础上，新增页面总览、Desktop / Tablet / Mobile 预览尺寸、发布前检查、草稿对线上差异摘要和发布确认变更摘要；页面总览显示 Home / About 的模块数、草稿数、未保存数、检查项和隐藏数；发布前检查覆盖隐藏模块、空模块、空条目、缺图片、缺链接和链接格式异常；差异摘要覆盖文字、图片、链接、显示状态、条目新增 / 删除 / 排序；`live_state` 用于草稿和线上版本对比，不改数据库结构。
...
- 08 可视化页面编辑器：C3 发布前检查与差异摘要已上线；当前仍不支持整页模块级新增 / 删除 / 排序、自由样式 / 自由布局；真实图片上传和无引用媒体删除成功路径仍需单独授权测试；下一步先补 08 运营使用规范，不继续扩大编辑能力；`/global` edge runtime warning 仍归 04 地图专项。
...
- 08 可视化页面编辑器：C3 发布前检查与差异摘要已上线；下一步先补 08 运营使用规范，再评估真实图片上传端到端、无引用媒体删除成功路径、更精细差异对比，以及页面级模块结构能力。
