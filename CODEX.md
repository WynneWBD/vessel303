...
- `/admin/products`：产品 CMS 已接入产品列表和通用详情页，支持新建、编辑、发布/下架、删除、筛选、图片选择和前台预览；固定精细详情页如 `e7`、`v9-gen6` 仍保留原页面。
...
- Step 8：产品 CMS 基础发布链路，含公开详情 slug 校验和后台预览。
...
- 产品 CMS 下一阶段：增加可编辑详情正文、图库、规格参数和多语言详情模块，减少对固定精细页的依赖。
