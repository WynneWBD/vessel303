...
性能 / 图片 / 前台速度：07 负责全站性能专项，包括首页首屏速度、产品列表打开速度、图片体积、前台缓存、Lighthouse / Chrome Network 基线和线上轻量测速。B8 已完成第一轮性能与图片治理：修复 `/admin/media` 嵌套 button 导致的 hydration 风险，公开产品列表 / 筛选 / 详情改为轻量缓存读取并避免公开路径触发 schema 初始化，首页 hero 改为只渲染当前图和下一张图并新增优化图片资产，后台媒体上传增加大图提示，产品写入后会刷新公开产品缓存。B13 已完成真实性能瓶颈定位与第一修复包：诊断确认当前核心不是 JS 主线程或服务端 TTFB，而是生产 `/about` 首屏直接加载 Vercel Blob 原图和导航点击体感不稳定；已允许 Vercel Blob 走 `next/image` 优化、About 图片改为可优化路径、Navbar 内部链接恢复 App Router `Link` 与 idle prefetch。线上 `/about` Lighthouse 复测：Performance 96、FCP 1.1s、LCP 2.7s、TBT 70ms、TTFB 33ms、总传输约 640KB，原 4.25MB Blob 首屏图已变为约 40KB 的 `/_next/image` WebP。产品中心体验问题由 01 配合，后台 API、CMS 查询或后台实现瓶颈由 02 处理，05 负责验收性能改动是否真实改善；后续图片缩略图派生、媒体库大图治理、全站点击体感和 Lighthouse 深测继续由 07 拆小步推进。
...
- 07 性能 / 图片 / 前台速度专项：B8 第一轮性能与图片治理已完成；B13 已完成真实性能瓶颈定位与第一修复包，确认生产 About 慢点主因是首屏原始 Blob 大图而不是 JS / TTFB，并将 Vercel Blob 图接入 `next/image` 优化。后续继续维护首页、产品中心和全站前台响应速度，按媒体缩略图派生、图片体积治理、Lighthouse 深测、缓存策略细化、后台产品页首屏速度和真实点击体感拆小步优化。工具故障和流程问题沉淀转入 00 / 06 文档规则管理。
...
- 后台运营体验 A / B 包：A1-A6、B1-B7、新 B6 运营数据中心、B6-7 访问分析准备页、B8 性能与图片治理第一轮、B9 固定内容 CMS 第一阶段、B10 产品中心路径闭环、B11 客户与线索后台 2.0 收口、B12 站点入口 / 表单路径收口和 B13 真实性能瓶颈第一修复包已上线；媒体真实上传 / 删除端到端测试仍需单独授权。
