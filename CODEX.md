...
- `/admin/projects`：项目 / 案例 CMS 已接入，支持新建、编辑、发布/下架、删除、筛选、封面图、图库排序、中英文案例内容、地图发布校验，以及 `/global` 详情里的设施亮点、交通指引和周边景点；前台 `/cases` 已优先读取数据库并保留静态兜底；带经纬度的已发布项目会进入 `/global` 地图点位和详情面板。
...
- V8.1：项目 CMS / 案例 CMS 的地图点位批量校验和更多详情字段模板。
...
- 项目 / 案例 CMS 下一阶段：继续完善地图点位批量校验和更多详情字段模板。
