...
- 08 可视化页面编辑器：`/admin/pages/visual`、页面模块可视化预览、受控字段编辑、模块高亮、点击定位、草稿预览 / 发布上线、发布前检查、差异摘要、快照恢复、模块内 item 管理、模块注册表 / 动态渲染基础和后续运营使用规范；不负责普通后台 A/B 包、产品 / 项目 CMS、`/global` 地图、会员支付。
...
- `/admin/pages/visual`：08 C4-1 动态渲染基础已完成；Home 的 `home:hero` / `home:credentials`，以及 About 的 10 个已接入 `page_modules` 的内容模块，已通过模块注册表 + `page_modules` 数据渲染；C4-1 是前台渲染架构基础，不是完整模块化建站器；当前仍不支持整页模块库、整页模块新增 / 删除 / 拖拽排序、自由布局 / 自由样式；Home 其他静态区块、About 的 Navbar / Footer / AnchorNav / certifications 外壳 / GlobalMapPreview / CTA 仍保持静态。
...
- 08 可视化页面编辑器：C4-1 动态渲染基础已完成；后续 C4-2 才能研究模块库与结构编辑，C4-2 前不要开放整页模块新增 / 删除 / 排序；当前仍不支持整页模块库、整页模块新增 / 删除 / 拖拽排序、自由布局 / 自由样式；`/global` edge runtime warning 仍归 04 地图专项。
...
- 08 可视化页面编辑器：C4-1 动态渲染基础已完成；下一步先做 C4-2 模块库与结构编辑只读方案，明确模块注册表、默认内容、可新增 / 不可删除规则、页面级结构草稿 / 发布、结构快照 / 恢复、结构变更摘要和前台 fallback / 坏数据处理。
