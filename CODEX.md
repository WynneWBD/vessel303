...
- 08 可视化页面编辑器：`/admin/pages/visual`、页面模块可视化预览、受控字段编辑、模块高亮、点击定位、草稿预览 / 发布上线、发布前检查、差异摘要、快照恢复、模块内 item 管理、模块注册表 / 动态渲染基础、只读模块库和后续运营使用规范；不负责普通后台 A/B 包、产品 / 项目 CMS、`/global` 地图、会员支付。
...
- `/admin/pages/visual`：08 C4-2a 已上线，只读模块库展示可规划模块、锁定模块、暂不开放模块、适用页面和限制原因；当前没有“添加 / 新增 / 使用此模块”按钮，没有开放整页模块新增 / 删除 / 拖拽排序；C4-2a 不写数据库、不保存结构草稿、不发布结构变更；后续 C4-2b 才能研究页面级结构草稿，C4-2b 前不要开放整页模块新增 / 删除 / 排序。
...
- 08 可视化页面编辑器：C4-2a 只读模块库展示已上线；当前仍不支持整页模块新增 / 删除 / 拖拽排序、结构草稿、结构发布、自由布局 / 自由样式；本机 Turbopack `os error 5` 属于本地构建环境问题，本轮采用 `next build --webpack` + Vercel READY 验证；`/global` edge runtime warning 仍归 04 地图专项。
...
- 08 可视化页面编辑器：C4-2a 只读模块库展示已上线；下一步先做 C4-2b 页面级结构草稿方案，重点研究新增 / 排序 / 删除 / 隐藏如何只存在于草稿、发布时如何一次性写入线上结构、页面级结构快照如何设计，以及如何避免后台新增但前台不显示的假功能。
