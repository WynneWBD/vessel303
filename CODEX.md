...
- 08 可视化页面编辑器：`/admin/pages/visual`、页面模块可视化预览、受控字段编辑、模块高亮、点击定位、后续版本快照 / 回滚方案；不负责普通后台 A/B 包、产品 / 项目 CMS、`/global` 地图、会员支付。
...
- `/admin/pages/visual`：08 C1-2 受控可视化编辑器 MVP 已上线，支持 Home / About 真实 iframe 预览、模块点击定位、高亮、右侧字段编辑、文字 / 链接 / `image_url` 编辑、图片库选择回填、模块显示 / 隐藏 UI、未保存提醒、保存前确认、保存当前模块；支持 `label` / `value` / `content` / `href` / `image_url` 受控字段编辑，图片库可回填 `image_url`；保存当前模块会立即影响前台页面。
...
- 08 可视化页面编辑器：已进入 C1-2，`/admin/pages/visual` 已从只读预览升级为受控可视化编辑器 MVP；已通过一次真实文字保存并恢复测试，最终前台未残留测试文案；图片真实上传端到端未做，模块隐藏未做真实保存；当前仍不支持新增模块、删除模块、拖拽排序、自由样式、自由布局、草稿 / 发布、版本回滚；下一步优先 C1-3 版本快照 / 回滚，而不是继续扩大编辑能力；`/global` edge runtime warning 仍归 04 地图专项。
...
- 08 可视化页面编辑器：C1-2 受控编辑 MVP 已上线；下一步优先 C1-3 版本快照 / 回滚，再评估模块新增、删除、排序。
