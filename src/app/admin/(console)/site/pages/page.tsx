...
const SITE_PAGES: SitePage[] = [
  {
    ...
    group: '页面编辑',
    source: '页面可视化编辑器',
    detail: '首页 Hero、核心数据和安全插入区由受控模块维护；B21 已补产品目录、项目案例和咨询路径的 300 对齐入口，草稿不会直接影响前台。',
    status: 'editable',
  ...
  },
  {
    ...
    group: '页面编辑',
    source: '页面可视化编辑器',
    detail: '关于页品牌、工厂、历程和荣誉等模块可编辑；B21 已把品牌判断路径继续接到产品中心和咨询入口，但不开放新增结构。',
    status: 'editable',
    editorHref: '/admin/site/visual',
    viewHref: '/about',
    Icon: FileText,
  ...
  },
  {
    ...
    group: '固定内容 CMS',
    source: 'FAQ CMS',
    detail: '常见问题分类、排序和发布状态由固定内容 CMS 维护；B21 补充当前显示数量、后台 owner 和线索承接提示，前台保留静态兜底。',
    status: 'managed',
...
export default async function AdminSitePagesPage() {
  ...
  return (
    <AdminSectionShell
      ...
      title="网站管理"
      description="按页面查看可编辑范围、内容来源、草稿状态、前台入口和 B21 精修边界。"
