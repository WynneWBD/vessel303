...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  if (parsed.data.slug) {
    const taken = await isProductCategorySlugTaken(parsed.data.slug, id)
    if (taken) {
      return NextResponse.json({ error: 'Category slug already in use' }, { status: 409 })
    }
  }

  const updated = await updateProductCategory(id, parsed.data)
  if (!updated) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  await logAdminAction(admin.id, 'product-category.update', 'product_categories', String(id))
