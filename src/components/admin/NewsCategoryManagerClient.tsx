...
import type { NewsCategoryRow, NewsCategoryStatus } from '@/lib/news-db'
...
function toFormState(category: NewsCategoryRow): CategoryFormState {
  return {
    slug: category.slug,
...
function statusLabel(status: NewsCategoryStatus) {
  return status === 'visible' ? '显示' : '隐藏'
}

function statusClass(status: NewsCategoryStatus) {
...
export default function NewsCategoryManagerClient({
  ...
  initialCategories: NewsCategoryRow[]
}) {
  ...
  const refreshCategories = async () => {
    const res = await fetch('/api/admin/news/categories?includeHidden=1', { cache: 'no-store' })
    if (!res.ok) throw new Error('分类刷新失败')
    const data = await res.json() as { data: NewsCategoryRow[] }
  ...
  const beginEdit = (category: NewsCategoryRow) => {
  ...
  const handleToggleStatus = async (category: NewsCategoryRow) => {
    const nextStatus: NewsCategoryStatus = category.status === 'visible' ? 'hidden' : 'visible'
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/news/categories/${category.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
              ...
              >
                {editing ? (
                  <>
                    ...
                    <span className="pt-2">{category.news_count ?? 0}</span>
                    <span>
                      <Select
                        value={editForm.status}
                        onChange={(e) => setEditForm((form) => ({ ...form, status: e.target.value as NewsCategoryStatus }))}
                      >
                        <option value="visible">显示</option>
                        <option value="hidden">隐藏</option>
                      </Select>
