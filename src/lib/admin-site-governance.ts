...
export const CONTENT_CONTRACTS: ContentContract[] = [
  ...
  },
  {
    ...
    displayRule: 'B28 后 /contact 渲染 contact 已发布模块，联系表单写入 leads；来源提示从 source-context 模块读取；客户确认邮件文案从 email 模块读取；contactUrl 仅作为备用旧站入口。',
  ...
  },
  {
    ...
    requiredModules: ['shared', 'login', 'register', 'header', 'profile', 'password'],
    signals: ['form', 'cta'],
    displayRule: '登录、注册和账户中心客户可见文案来自 auth/account 页面模块；只治理展示文案，不改认证、权限或账户安全逻辑。',
  ...
  },
  {
    ...
    signals: ['seo', 'cta', 'english'],
    displayRule: 'Global 前台展示文案从 page_modules:global 读取；地图底层、点位算法和 /api/map 仍按地图专项保护。',
    hiddenRule: '模块关闭时使用 Global 页面默认兜底文案，地图底层问题回 04 专项。',
    note: '本批只开放标题、数字区、加载/详情/CTA/SEO 文案编辑，不改 MapLibre、MapTiler、点位算法或项目数据。',
